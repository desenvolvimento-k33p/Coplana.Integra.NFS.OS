CREATE OR REPLACE VIEW COPLANA_PRD.K33P_TRANS_NF_ENTRADA AS
SELECT *
FROM
(SELECT
            DISTINCT '' AS "DocNumPedTransf",
            IFNULL(OWTR."DocNum", 0) AS "DocNumTransf",
            ...
            TO_VARCHAR(OINV."DocDueDate", 'YYYY-MM-DD') AS "DocDueDate",
        ...
        INNER JOIN NFN1 ON NFN1."BPLId" = OBPL."BPLId" AND NFN1."Model" = 39 AND NFN1."Locked" = 'N' 
        INNER JOIN OWHS ON WTR1."WhsCode" = OWHS."WhsCode" 
        LEFT JOIN OINV ON OWTR."DocNum" = OINV."U_NumTransf" AND OINV.CANCELED = 'N' AND OINV."DocEntry" = ( SELECT MAX(OINV_MAX."DocEntry") FROM OINV OINV_MAX INNER JOIN OFPR T0 ON OINV_MAX."FinncPriod" = T0."AbsEntry" AND T0."PeriodStat" = 'N' WHERE OWTR."DocNum" = OINV_MAX."U_NumTransf" AND OINV_MAX.CANCELED = 'N' ) 
        LEFT JOIN OPCH ON OWTR."DocNum" = OPCH."U_NumTransf" AND OPCH.CANCELED = 'N' AND OPCH."DocEntry" = ( SELECT MAX(OPCH_MAX."DocEntry") FROM OPCH OPCH_MAX INNER JOIN OFPR T0 ON OPCH_MAX."FinncPriod" = T0."AbsEntry" AND T0."PeriodStat" = 'N' WHERE OWTR."DocNum" = OPCH_MAX."U_NumTransf" AND OPCH_MAX.CANCELED = 'N' ) 
        INNER JOIN (
            SELECT  
            ...
            INNER JOIN "SBO_TaxOne"."Entidade" ON "DBInvOne"."Process"."CompanyId" = "SBO_TaxOne"."Entidade".ID AND "SBO_TaxOne"."Entidade"."CompanyDb" = CURRENT_SCHEMA 
            INNER JOIN "DBInvOne"."ProcessHist" ON "DBInvOne"."Process"."BatchId" = "DBInvOne"."ProcessHist" ."BatchId" AND "DBInvOne"."ProcessHist"."Id" = ( SELECT MAX(PRAHIS."Id") FROM "DBInvOne"."ProcessHist" PRAHIS WHERE "DBInvOne"."ProcessHist"."BatchId" = PRAHIS."BatchId" )
            WHERE "DBInvOne"."Process"."DocType" = 13
        ) PRO ON PRO."DocEntry" = OINV."DocEntry" AND PRO."DocType" = OINV."ObjType" 
        LEFT JOIN "@K33P_TRAN_PADC" ON OBPL."BPLId" = "@K33P_TRAN_PADC"."U_FilialSai" AND OWHS."U_FilDePara" = "@K33P_TRAN_PADC"."U_FilialEnt" 
    ...
    GROUP BY 
        ...
        TO_VARCHAR(OINV."DocDate", 'YYYY-MM-DD'),
        TO_VARCHAR(OINV."TaxDate", 'YYYY-MM-DD'),
        TO_VARCHAR(OINV."DocDueDate", 'YYYY-MM-DD'),
        ...
        SELECT
            DISTINCT IFNULL(TO_NVARCHAR( OINV."DocEntry"), '') AS "DocNumPedTransf",
        ...
        AND OBPL."BPLId" NOT IN (21,    6,  24 , 15) 
        THEN 'Y' 
        ELSE 'N' 
        ...
        INNER JOIN OFPR OFPR ON OINV."FinncPriod" = OFPR."AbsEntry"  
        INNER JOIN ( SELECT
     "DBInvOne"."Process"."DocEntry" ,
     "DBInvOne"."Process"."DocType" ,
     "SBO_TaxOne"."Entidade"."BusinessPlaceId" ,
    ...
    TO_VARCHAR(OINV."DocDueDate", 'YYYY-MM-DD'),
     ...
     OUSG."U_K33P_EscrituraNF") AS RESULTADO 
WHERE NOT EXISTS (
    SELECT 1
    FROM KEEPLOGS.KEEP_LOG_NFS_OS LOG 
    WHERE SUBSTR_BEFORE(SUBSTR_AFTER(TO_VARCHAR(LOG.REQUESTOBJECT),'"U_ChaveAcesso":"'),'",') = RESULTADO."U_ChaveAcesso"
    AND SECONDS_BETWEEN(LOG.LOGHOUR,CURRENT_TIME) < 3600
    AND LOG.KEY_PARC = '0'
    )
ORDER BY RESULTADO."DocDate" DESC;
