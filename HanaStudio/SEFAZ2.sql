...
CASE WHEN IFNULL(T1."BaseRef",'') <> '' then  (select MAX("DocEntry") from OINV WHERE "U_NumPedTr" = T1."BaseRef" AND "CANCELED" = 'N') 
	 WHEN IFNULL(T1."BaseRef",'') = '' then (select MAX("DocEntry") from OINV WHERE "U_NumTransf" = T0."DocNum" AND "CANCELED" = 'N')  END
AND DB."DocType" = 13
